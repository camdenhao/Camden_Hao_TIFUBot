...
bot = commands.Bot(command_prefix = '.') #all bot commands must start with '.'

@bot.event
async def on_ready():
    guild = discord.utils.get(bot.guilds, name = 'CH_DALI_TEMP')
    print(f'{bot.user.name} has connected to Discord!')
@bot.command(pass_context = True) 
async def read(ctx, link):  #read command - main function of the bot 
    if 'reddit.com/r/tifu' in link:
        try:
            submission = reddit.submission(url=link) #accesses submission
            try:
                ...
                if len(bot.voice_clients) == 0: #verifies if bot is in voice channel. If not, then joins the channel the user is in
                    ...
                    vc = bot.voice_clients[0] #declars the voice_client 
                    #can't start new audio without stopping current one 
                    if vc.is_playing():       
                        vc.stop() 
